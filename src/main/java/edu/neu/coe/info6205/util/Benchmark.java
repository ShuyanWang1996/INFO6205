...
public class Benchmark<T> {
    ...
    public static void main(String[] args) {
        ...
        for (int k = 0; k < 7; k++) {
            ...
            //2. Ordered array
            Helper helper = new Helper("Creating Array");
            InsertionSort sorter = new InsertionSort(helper);
            Integer[] arrayOrdered = new Integer[n];
            for (int i = 0; i < n; i++) arrayOrdered[i] = array[i];
            sorter.sort(arrayOrdered);
            //3. Partially-Ordered array
            Integer[] arrayPartiallyOrdered = new Integer[n];
            for (int i = 0; i < n/2; i++) arrayPartiallyOrdered[i] = arrayOrdered[i];
            for (int i = n/2; i < n; i++) arrayPartiallyOrdered[i] = array[i];
            //4. Reverse array
            Integer[] arrayReversed = new Integer[n];
            for (int i = 0; i < n; i++) arrayReversed[i] = arrayOrdered[n-i-1];
            //TODO Choose from among the following
            benchmarkSort(array, "InsertionSort: " + n + " Random" , new InsertionSort<>(), m);
            benchmarkSort(arrayOrdered, "InsertionSort: " + n + " Ordered" , new InsertionSort<>(), m);
            benchmarkSort(arrayPartiallyOrdered, "InsertionSort: " + n + " PartiallyOrdered" , new InsertionSort<>(), m);
            benchmarkSort(arrayReversed, "InsertionSort: " + n + " Reverse" , new InsertionSort<>(), m);
